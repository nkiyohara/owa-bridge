...
The server exposes twenty tools:
...
- `calendar_create`: prepares one exact plain-text, non-recurring, non-all-day
  event, optionally with a closed `teamsMeeting` boolean, and returns a
  caller-bound mandatory preview. It never creates an event or sends an
  invitation directly.
...
- `calendar_update`: prepares a stale-safe patch for one exact event ID and
  change key. Only subject, plain-text body, start plus end, and location are
  accepted; an empty provided string clears that field.
...
- `mail_get_body`: bounded plain text for one exact message ID. It returns
  content immediately under the default policy or an in-memory approval
  preview when sensitive-read previews are enabled.
- `mail_get_body_commit`: consumes that short-lived, caller-bound preview.
- `mail_create_draft`: creates one plain-text draft with OWA `SaveOnly`; it
  never sends mail. Policy may instead return a bounded review and approval
  token.
...
- `mail_send`: prepares a new plain-text message and returns its normalized
...
operations. The cancellation commit is destructive; its preview is not.
